...
RAW_PROBLEMS = [
    ...
    {"id": 184, "type": "COURSE", "moduleId": "08", "title": "Products Priced Above Average", "category": "1.7 Subqueries", "difficulty": "beginner",
     ...
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย >", "subquery หาค่า AVG(list_price) ของสินค้าทั้งหมด"]},
    ...
    {"id": 186, "type": "COURSE", "moduleId": "08", "title": "Product With Highest Price", "category": "1.7 Subqueries", "difficulty": "beginner",
     ...
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย =", "subquery หาค่า MAX(list_price) ของสินค้าทั้งหมด"]},
    ...
    {"id": 189, "type": "COURSE", "moduleId": "08", "title": "Order Items With Above Average Quantity", "category": "1.7 Subqueries", "difficulty": "intermediate",
     ...
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย >", "subquery หาค่า AVG(quantity) ของรายการสั่งซื้อทั้งหมด"]},
    {"id": 190, "type": "COURSE", "moduleId": "08", "title": "Products Above Category Average Price", "category": "1.7 Subqueries", "difficulty": "intermediate",
     "description": "จงเขียน SQL Statement แสดงสินค้าที่มีราคา (list_price) มากกว่าราคาเฉลี่ยของสินค้าในหมวดหมู่เดียวกัน โดยใช้ subquery แบบอ้างอิงไขว้กับตาราง products ของ query หลัก", "table": "products",
     ...
     "requirements": ["ใช้ subquery แบบอ้างอิงไขว้กับตาราง products ของ query หลัก (p1)", "subquery หาค่า AVG(list_price) เฉพาะสินค้าในหมวดหมู่เดียวกัน"]},
    ...
    {"id": 192, "type": "COURSE", "moduleId": "08", "title": "Products Not In Cheapest Category", "category": "1.7 Subqueries", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงสินค้าที่ไม่ได้อยู่ในหมวดหมู่เดียวกับสินค้าที่มีราคาถูกที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", "table": "products",
     "goldenQuery": "SELECT * FROM products WHERE category_id <> (SELECT category_id FROM products ORDER BY list_price ASC LIMIT 1);", "starterCode": "SELECT ",
     "requirements": ["ใช้ตัวดำเนินการ <> เปรียบเทียบค่ากับ subquery", "subquery เรียง list_price จากน้อยไปมากและใช้ LIMIT 1 เพื่อให้ได้ค่าเดียว"]},
    {"id": 193, "type": "COURSE", "moduleId": "08", "title": "Staff Not In Busiest Store", "category": "1.7 Subqueries", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงพนักงานที่ไม่ได้สังกัดร้านค้าที่มีจำนวนพนักงานมากที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", "table": "staffs",
     "goldenQuery": "SELECT * FROM staffs WHERE store_id <> (SELECT store_id FROM staffs GROUP BY store_id ORDER BY COUNT(*) DESC LIMIT 1);", "starterCode": "SELECT ",
     "requirements": ["ใช้ตัวดำเนินการ <> เปรียบเทียบค่ากับ subquery", "subquery จัดกลุ่มด้วย GROUP BY แล้วเรียงจำนวนพนักงานมากไปน้อยพร้อม LIMIT 1"]},
    {"id": 194, "type": "COURSE", "moduleId": "08", "title": "Orders By Staff Of Latest Order", "category": "1.7 Subqueries", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงคำสั่งซื้อทั้งหมดของพนักงานที่รับผิดชอบคำสั่งซื้อล่าสุด (order_date ล่าสุด) โดยใช้ subquery แบบค่าเดียว ในเงื่อนไข WHERE", "table": "orders",
     ...
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย =", "subquery เรียง order_date จากล่าสุดและใช้ LIMIT 1 เพื่อให้ได้ค่าเดียว"]},
    {"id": 195, "type": "COURSE", "moduleId": "08", "title": "Categories With Above Average Product Price", "category": "1.7 Subqueries", "difficulty": "advanced",
     ...
     "requirements": ["ใช้ subquery ใน FROM (derived table) หาราคาเฉลี่ยของแต่ละหมวดหมู่", "ใช้ JOIN เชื่อม derived table กับตาราง categories", "ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบกับราคาเฉลี่ยรวม"]},
    ...
    {"id": 196, "type": "ASSIGNMENT", "moduleId": "08", "title": "Stocks Below Average Quantity", "category": "2.7 Subqueries", "difficulty": "beginner",
     ...
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย <", "subquery หาค่า AVG(quantity) ของสินค้าคงคลังทั้งหมด"]},
    ...
    {"id": 198, "type": "ASSIGNMENT", "moduleId": "08", "title": "Cheapest Product", "category": "2.7 Subqueries", "difficulty": "beginner",
     ...
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย =", "subquery หาค่า MIN(list_price) ของสินค้าทั้งหมด"]},
    ...
    {"id": 201, "type": "ASSIGNMENT", "moduleId": "08", "title": "Order Items Above Average Discount", "category": "2.7 Subqueries", "difficulty": "intermediate",
     ...
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย >", "subquery หาค่า AVG(discount) ของรายการสั่งซื้อทั้งหมด"]},
    {"id": 202, "type": "ASSIGNMENT", "moduleId": "08", "title": "Items Above Their Order Average Quantity", "category": "2.7 Subqueries", "difficulty": "intermediate",
     "description": "จงเขียน SQL Statement แสดงรายการสั่งซื้อที่มีจำนวน (quantity) มากกว่าค่าเฉลี่ยของรายการในคำสั่งซื้อเดียวกัน โดยใช้ subquery แบบอ้างอิงไขว้ ที่อ้างอิง order_id ของ query หลัก", "table": "order_items",
     ...
     "requirements": ["ใช้ subquery แบบอ้างอิงไขว้ ที่อ้างอิง order_id ของ query หลัก (oi1)", "subquery หาค่า AVG(quantity) เฉพาะรายการในคำสั่งซื้อเดียวกัน"]},
    ...
    {"id": 204, "type": "ASSIGNMENT", "moduleId": "08", "title": "Products Not From Top Brand", "category": "2.7 Subqueries", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงสินค้าที่ไม่ได้อยู่ในยี่ห้อที่มีจำนวนสินค้ามากที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", "table": "products",
     "goldenQuery": "SELECT * FROM products WHERE brand_id <> (SELECT brand_id FROM products GROUP BY brand_id ORDER BY COUNT(*) DESC LIMIT 1);", "starterCode": "SELECT ",
     "requirements": ["ใช้ตัวดำเนินการ <> เปรียบเทียบค่ากับ subquery", "subquery จัดกลุ่มด้วย GROUP BY แล้วเรียงจำนวนสินค้ามากไปน้อยพร้อม LIMIT 1"]},
    {"id": 205, "type": "ASSIGNMENT", "moduleId": "08", "title": "Stores Not Matching Top Seller", "category": "2.7 Subqueries", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงร้านค้าที่ไม่ใช่ร้านที่มีจำนวนคำสั่งซื้อมากที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", "table": "stores",
     "goldenQuery": "SELECT * FROM stores WHERE store_id <> (SELECT store_id FROM orders GROUP BY store_id ORDER BY COUNT(*) DESC LIMIT 1);", "starterCode": "SELECT ",
     "requirements": ["ใช้ตัวดำเนินการ <> เปรียบเทียบค่ากับ subquery", "subquery จัดกลุ่มด้วย GROUP BY แล้วเรียงจำนวนคำสั่งซื้อมากไปน้อยพร้อม LIMIT 1"]},
    {"id": 206, "type": "ASSIGNMENT", "moduleId": "08", "title": "Order With Largest Single Item Quantity", "category": "2.7 Subqueries", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงรายการสั่งซื้อของคำสั่งซื้อ (order_id) ที่มีจำนวนสินค้า (quantity) เท่ากับจำนวนสินค้าที่สั่งมากที่สุดในรายการเดียว โดยใช้ subquery แบบค่าเดียว ในเงื่อนไข WHERE", "table": "order_items",
     "goldenQuery": "SELECT * FROM order_items WHERE quantity = (SELECT MAX(quantity) FROM order_items);", "starterCode": "SELECT ",
     "requirements": ["ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบด้วย =", "subquery หาค่า MAX(quantity) ของรายการสั่งซื้อทั้งหมด"]},
    {"id": 207, "type": "ASSIGNMENT", "moduleId": "08", "title": "Stores With Above Average Total Stock", "category": "2.7 Subqueries", "difficulty": "advanced",
     ...
     "requirements": ["ใช้ subquery ใน FROM (derived table) รวมจำนวนคงเหลือของแต่ละร้าน", "ใช้ JOIN เชื่อม derived table กับตาราง stores", "ใช้ subquery แบบค่าเดียว ใน WHERE เปรียบเทียบกับค่าเฉลี่ย"]},
    ...
    {"id": 208, "type": "EXAM", "moduleId": "08", "title": "Orders Not From Top Staff", "category": "3.8 EXAM", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงคำสั่งซื้อทั้งหมดที่ไม่ได้มาจากพนักงานที่มียอดขายรวม (list_price คูณ quantity) สูงที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", "table": "orders",
     "goldenQuery": "SELECT * FROM orders WHERE staff_id <> (SELECT o.staff_id FROM orders o JOIN order_items oi ON o.order_id = oi.order_id GROUP BY o.staff_id ORDER BY SUM(oi.list_price * oi.quantity) DESC LIMIT 1);", "starterCode": "SELECT ",
     "requirements": ["ใช้ตัวดำเนินการ <> เปรียบเทียบค่ากับ subquery", "subquery เชื่อม orders กับ order_items แล้วจัดกลุ่มหายอดขายรวมต่อพนักงาน เรียงมากไปน้อยพร้อม LIMIT 1"]},
    {"id": 209, "type": "EXAM", "moduleId": "08", "title": "Best Selling Product Per Category", "category": "3.8 EXAM", "difficulty": "advanced",
     "description": "จงเขียน SQL Statement แสดงชื่อหมวดหมู่ (category_name) พร้อมรหัสสินค้า (product_id) ที่มีผลรวมจำนวนขาย (quantity) สูงที่สุดในหมวดหมู่นั้น โดยใช้ subquery ใน FROM (derived table) หาผลรวมจำนวนขายของแต่ละสินค้าก่อน แล้วใช้ subquery แบบอ้างอิงไขว้ เปรียบเทียบค่าสูงสุดในแต่ละหมวดหมู่", "table": "categories",
     ...
     "requirements": ["ใช้ subquery ใน FROM (derived table) รวมยอดขายของแต่ละสินค้า", "ใช้ subquery แบบอ้างอิงไขว้ เปรียบเทียบยอดขายสูงสุดในหมวดหมู่เดียวกัน", "ใช้ JOIN เชื่อม 3 ตาราง (categories, products, order_items)"]},
]
...
MODULE_DEFS = {
    ...
    "08": {"title": "Subqueries", "description": "คำสั่งย่อยแบบค่าเดียว แบบอ้างอิงไขว้ และ derived table", "order": 7},
}

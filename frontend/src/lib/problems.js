...
const rawProblems = [
  ...
  { id: 190, tutorProblemId: 333, type: "COURSE", moduleId: "08", title: "Products Above Category Average Price", category: "1.7 Subqueries", difficulty: "intermediate",
    description: "จงเขียน SQL Statement แสดงสินค้าที่มีราคา (list_price) มากกว่าราคาเฉลี่ยของสินค้าในหมวดหมู่เดียวกัน โดยใช้ subquery แบบอ้างอิงไขว้กับตาราง products ของ query หลัก", table: "products", goldenQuery: "SELECT * FROM products p1 WHERE list_price > (SELECT AVG(list_price) FROM products p2 WHERE p2.category_id = p1.category_id);", starterCode: "SELECT ",
  ...
  { id: 192, tutorProblemId: 335, type: "COURSE", moduleId: "08", title: "Products Not In Cheapest Category", category: "1.7 Subqueries", difficulty: "advanced",
    description: "จงเขียน SQL Statement แสดงสินค้าที่ไม่ได้อยู่ในหมวดหมู่เดียวกับสินค้าที่มีราคาถูกที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", table: "products", goldenQuery: "SELECT * FROM products WHERE category_id <> (SELECT category_id FROM products ORDER BY list_price ASC LIMIT 1);", starterCode: "SELECT ",
  ...
  { id: 193, tutorProblemId: 336, type: "COURSE", moduleId: "08", title: "Staff Not In Busiest Store", category: "1.7 Subqueries", difficulty: "advanced",
    description: "จงเขียน SQL Statement แสดงพนักงานที่ไม่ได้สังกัดร้านค้าที่มีจำนวนพนักงานมากที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", table: "staffs", goldenQuery: "SELECT * FROM staffs WHERE store_id <> (SELECT store_id FROM staffs GROUP BY store_id ORDER BY COUNT(*) DESC LIMIT 1);", starterCode: "SELECT ",
  ...
  { id: 194, tutorProblemId: 337, type: "COURSE", moduleId: "08", title: "Orders By Staff Of Latest Order", category: "1.7 Subqueries", difficulty: "advanced",
    description: "จงเขียน SQL Statement แสดงคำสั่งซื้อทั้งหมดของพนักงานที่รับผิดชอบคำสั่งซื้อล่าสุด (order_date ล่าสุด) โดยใช้ subquery แบบค่าเดียว ในเงื่อนไข WHERE", table: "orders", goldenQuery: "SELECT * FROM orders WHERE staff_id = (SELECT staff_id FROM orders ORDER BY order_date DESC LIMIT 1);", starterCode: "SELECT ",
  ...
  { id: 202, tutorProblemId: 381, type: "ASSIGNMENT", moduleId: "08", title: "Items Above Their Order Average Quantity", category: "2.7 Subqueries", difficulty: "intermediate",
    description: "จงเขียน SQL Statement แสดงรายการสั่งซื้อที่มีจำนวน (quantity) มากกว่าค่าเฉลี่ยของรายการในคำสั่งซื้อเดียวกัน โดยใช้ subquery แบบอ้างอิงไขว้กับ order_id ของ query หลัก", table: "order_items", goldenQuery: "SELECT * FROM order_items oi1 WHERE quantity > (SELECT AVG(quantity) FROM order_items oi2 WHERE oi2.order_id = oi1.order_id);", starterCode: "SELECT ",
    requirements: ["ใช้ subquery แบบอ้างอิงไขว้กับ order_id ของ query หลัก (oi1)", "subquery หาค่า AVG(quantity) เฉพาะรายการในคำสั่งซื้อเดียวกัน"] },
  ...
  { id: 204, tutorProblemId: 383, type: "ASSIGNMENT", moduleId: "08", title: "Products Not From Top Brand", category: "2.7 Subqueries", difficulty: "advanced",
    description: "จงเขียน SQL Statement แสดงสินค้าที่ไม่ได้อยู่ในยี่ห้อที่มีจำนวนสินค้ามากที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", table: "products", goldenQuery: "SELECT * FROM products WHERE brand_id <> (SELECT brand_id FROM products GROUP BY brand_id ORDER BY COUNT(*) DESC LIMIT 1);", starterCode: "SELECT ",
  ...
  { id: 205, tutorProblemId: 384, type: "ASSIGNMENT", moduleId: "08", title: "Stores Not Matching Top Seller", category: "2.7 Subqueries", difficulty: "advanced",
    description: "จงเขียน SQL Statement แสดงร้านค้าที่ไม่ใช่ร้านที่มีจำนวนคำสั่งซื้อมากที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", table: "stores", tables: ["stores", "orders"], goldenQuery: "SELECT * FROM stores WHERE store_id <> (SELECT store_id FROM orders GROUP BY store_id ORDER BY COUNT(*) DESC LIMIT 1);", starterCode: "SELECT ",
  ...
  // --- 3.8 EXAM (Module: 08) ---
  { id: 208, tutorProblemId: 393, type: "EXAM", moduleId: "08", title: "Orders Not From Top Staff", category: "3.8 EXAM", difficulty: "advanced",
    description: "จงเขียน SQL Statement แสดงคำสั่งซื้อทั้งหมดที่ไม่ได้มาจากพนักงานที่มียอดขายรวม (list_price คูณ quantity) สูงที่สุด โดยใช้ตัวดำเนินการ <> เปรียบเทียบกับ subquery แบบค่าเดียว", table: "orders", tables: ["orders", "order_items"], goldenQuery: "SELECT * FROM orders WHERE staff_id <> (SELECT o.staff_id FROM orders o JOIN order_items oi ON o.order_id = oi.order_id GROUP BY o.staff_id ORDER BY SUM(oi.list_price * oi.quantity) DESC LIMIT 1);", starterCode: "SELECT ",
